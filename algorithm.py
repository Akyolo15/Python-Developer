...
teams=["Fenerbahçe","Galatasaray","Adana","Konyaspor","İstanbulspor","Ümraniye"]
teams_yedek=["Fenerbahçe","Galatasaray","Adana","Konyaspor","İstanbulspor","Ümraniye"]
size_team=len(teams)
if len(teams)%2!=0:
    teams.append("X")
Strength=[]
...
if (size_team % 2) != 0: #\\ Ligin ilk yarısı
    for t in range(int(len(eslesmeler1)/int((size_team+1)/2))):
        ...
        for i in range(len(tested)-1):
            ...
            sorted_name_team.append(name_team[enk_indis])
...
if (size_team % 2) != 0: #\\ Ligin ikinci yarısı
    for t in range(int(len(eslesmeler2)/int((size_team+1)/2))):
        ...
        for i in range(len(average)):
            ...
            if i < len(average)-1:
                ...
                ratio2.append(ratio[i] + (float(ratio[i]) * (1 - float(ratio[i])) * (1 / ((len(average) / 4) * (1 + (len(average) / 4)))) * (i - len(average) / 2)))
    ...
    for t in range(int(len(eslesmeler1)/int((size_team)/2))):
        ...
        for i in range(len(average)):
            ind_points = list(taken_point)
            son = int(sonuc[0][4*i+1])
            ind_po= ind_points.index(son)
            name_team[ind_po] = name_team[i]
            ind = tested.index(0.65*int(puan_durumu[i])+int(average[i])*0.3+0.05*int(goal_number[i]))
            sorted_average[ind]=int(average[i])
            sorted_goal_number[ind]=int(goal_number[i])
            tested2.append(int(sorted_goal_number[ind]))
